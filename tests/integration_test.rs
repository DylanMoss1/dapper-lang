use dapper::{Expr, LexerBridge, Module, ModuleParser, Opcode, Token, TypeChecker, Var};
use logos::Logos;
...
fn test_simple_function_no_params() {
    ...
    assert_eq!(module.0[0].name, Var("answer".to_string()));
...
fn test_multiple_functions_in_module() {
    ...
    assert_eq!(module.0.len(), 3);
    assert_eq!(module.0[0].name, Var("add".to_string()));
    assert_eq!(module.0[1].name, Var("sub".to_string()));
    assert_eq!(module.0[2].name, Var("main".to_string()));
}
...
fn test_generic_and_non_generic_functions() {
    ...
    assert_eq!(module.0[0].type_params.len(), 1);
    assert_eq!(module.0[1].type_params.len(), 0);
    assert_eq!(module.0[2].type_params.len(), 0);
}
